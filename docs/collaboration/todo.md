...
| 009 | 去掉 GLOBAL_CANVAS_STATE（per-request context） | codex-B | backlog | 007,014,015 | 2025-12-27 | P0：移除服务端全局可变 state，避免未来并发/多 tab 测试串线；把 tool context 变为 request-scoped |
...
| 016 | 下线 /api/ggb，统一走 /api/chat | codex-B | backlog | 015 | 2025-12-27 | 避免两套 API 漂移；短期可做转发，最终删除旧路由 |
