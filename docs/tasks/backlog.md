...
- [x] **Stability: canvas state on-demand** — 默认不发送 `canvasState`；引用/修改现有图时通过 `get_canvas_state()` 的 tool runner 按需获取（服务端 `tool_request` 兜底）。
- [x] **Stability: tool vs commands boundary** — 服务端/协议侧禁止把 `get_canvas_state()` 混进 GeoGebra `commands`，并在必要时强制走 tool-calling 链路。
- [x] **Tech debt: remove GLOBAL_CANVAS_STATE** — 去掉服务端全局可变 state（为未来并发与可测性铺路）。
